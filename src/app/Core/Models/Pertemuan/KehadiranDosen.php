...
use App\Core\Exception\KehadiranDosenException;
...
class KehadiranDosen
{
    ...
    public static function hadir(
        ...
        if (!$pertemuan->getStatus()->isSedangBerlangsung()) {
            throw new KehadiranDosenException('pencatatan_kehadiran_dosen_hanya_dapat_dilakukan_ketika_kelas_sedang_berlangsung');
        }

        return new KehadiranDosen(
            new KehadiranDosenId($pertemuan->getId(), $dosenId),
            $jamMulai,
            null,
            false,
            $bentukHadir
